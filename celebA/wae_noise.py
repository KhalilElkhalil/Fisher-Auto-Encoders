...
for noise_idx in range(sigma2.shape[0]):
    ...
    out_dir = "out_aae3_noise{}".format(noise_idx)
    s = sigma2[noise_idx] 
    ...
    for epoch in range(21):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            data = batch_item[0] + torch.sqrt(s) * torch.randn_like(batch_item[0])
            X = Variable(data)
            if cuda:
                X = X.cuda()
            ...
            if batch_idx == 0:
                ...
                with torch.no_grad():
                    ...
                    for batch_v, _ in valid_loader:
                        data = Variable(batch_v, requires_grad=True).to('cuda:0')
                        z = Q(data)
                        output = P(z)
                        mse_total += F.mse_loss(output, batch_v.to('cuda:0'))
                    print(' MSE : {} '.format(mse_total / len(valid_loader)))
                ...
                if score_fisher < best_FID:
                    ...
                    torch.save(Q,"Q_best_{:.1f}_noise{}.pth".format(score_fisher,s))
                    torch.save(P,"P_best_{:.1f}_noise{}.pth".format(score_fisher,s))
                    torch.save(D,"D_best_{:.1f}_noise{}.pth".format(score_fisher,s))
...
with open('wae_noise_FID.pkl','wb') as f:
    pickle.dump(all_fid, f)
with open('wae_noise_mse.pkl','wb') as f:
