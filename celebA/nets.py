...
class VAE(nn.Module):
    ...
    def forward(self, x, detach=False, test=False):
        if test:
        ...
        if self.flow:
            # sample from the new latent 
            # this is a transformation of the original Gaussian
            z_out = self.flow.sample(noise = z)

            pz0 = torch.distributions.normal.Normal(0., 1.) # prior dist. 
            KL = q0.log_prob(z).sum() - pz0.log_prob(z).sum() # KL[q(z|x) || p(z)]
        else:
            z_out = z
        
        # decode 
        x_hat = self.decode(z_out)

        if self.Fisher is True:

            if self.flow:
                dlnqzx = grad(self.flow.log_probs(z_out).sum(), x, create_graph=True)[0]
                dlnqzz = grad(self.flow.log_probs(z_out).sum(), z_out, create_graph=True)[0]
            else:
                dlnqzx = grad(q0.log_prob(z_out).sum(), x, create_graph=True)[0] # d/dx log q(z|x)
                dlnqzz = grad(q0.log_prob(z_out).sum(), z_out, create_graph=True)[0] # d/dz log q(z|x)
            ...
            dlnpxz = grad(lnpxz.sum(), z_out, retain_graph=True)[0] # d/dz log p(x|z)
            
            if self.exp_family is True:
                dlnpz, _ = self.dlnpz_exp(z_out) # Exp. family prior 
            else:
                dlnpz = -z_out # Gaussian prior 
